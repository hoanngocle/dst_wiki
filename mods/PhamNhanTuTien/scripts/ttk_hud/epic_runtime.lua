AddPrefab("ttk_hud_epichealth_proxy")
...
if not TheNet:IsDedicated() then
	AddAsset("ATLAS", "images/ttk_hud_epichealthbar.xml")
	AddAsset("ANIM", "anim/quagmire_hangry_bar.zip")

	AddClassPostInit("widgets/controls", function(self, owner)
		local EpicHealthbar = require "widgets/ttk_hud_epichealthbar"
		self.epichealthbar = self.top_root:AddChild(EpicHealthbar(owner, modinfo, modname))
		self.epichealthbar:SetPosition(TUNING.TTK_HUD_EPIC.HORIZONTAL_OFFSET, 0)
	end)

	for prefab, fn in pairs(TUNING.TTK_HUD_EPIC.CONDITIONS) do
...
AddUserCommand("epic",
{
	aliases = { "epichealth", "ttk_hud_epichealthbar" },
...
local function MakeEpicTarget(inst)
	if inst.epichealth == nil and not inst.isplayer then
		inst.epichealth = inst:SpawnChild("ttk_hud_epichealth_proxy")
	end
end
...
AddComponentPostInit("health", function(self, inst)
	if TUNING.TTK_HUD_EPIC.GLOBAL then
	...
	elseif inst:HasAnyTag("largecreature", "hostile") then
		TTK_HUDCore.Before(inst, "AddTag", OnAddTag)
	end
end)
...
AddComponentPostInit("explosiveresist", function(self, inst)
	TTK_HUDCore.Before(self, "OnExplosiveDamage", OnExplosiveDamage)
end)
	...
	if data ~= nil and data.damageresolved ~= nil and (data.damageresolved > 0 or data.redirected) then
		...
		if checkentity(source) and source.isplayer and source ~= inst then
			SendModRPCToClient("ShowPopupNumber",
				...
				data.redirected and "redirected" or data.stimuli or TTK_HUDCore.Browse(data.weapon, "components", "weapon", "stimuli"),
...
local function OnNewTarget(inst, data)
	local other = TTK_HUDCore.Browse(data, "target", "components", "combat", "target")
...
AddComponentPostInit("combat", function(self, inst)
	if TUNING.TTK_HUD_EPIC.GLOBAL_NUMBERS then
		inst:ListenForEvent("attacked", OnAttacked)
		if inst:HasTag("companion") then
			inst:ListenForEvent("newcombattarget", OnNewTarget)
		end
		TTK_HUDCore.Before(self, "OnRemoveFromEntity", OnRemoveFromEntity)
	end
end)
