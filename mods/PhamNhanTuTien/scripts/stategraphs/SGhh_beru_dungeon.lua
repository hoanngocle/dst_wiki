...
local STRONG_CD = 27
local SWIPE_ARC = 100
local SWIPE_OFFSET = 1.3
local SWIPE_RADIUS = 3
...
local function eventAttackSg(inst, data)
    ...
    elseif not inst["components"]["timer"]:TimerExists("pig_strong_cd") then
        -- Kỹ năng 2: Cường hoá thuộc tính (Kích hoạt mỗi 27s - Gầm thét tăng tốc)
        inst_sg:GoToState("strong", hh_target)
        return true
    elseif not inst["components"]["timer"]:TimerExists("pig_control_cd") then
        -- Kỹ năng 3: Khống chế diện rộng (Kích hoạt mỗi 45s - Gõ giáp gây choáng)
...
local hh_states = {
    ...
    State {
        ...
        ["onenter"] = function(inst)
            ...
            -- Đảm bảo chắc chắn sinh xác hh_corpse_beru dù animation bị ngắt hoặc không phát event
            if not inst._corpse_spawn_task then
                inst._corpse_spawn_task = inst:DoTaskInTime(3.5, function(inst)
                    if not inst._corpse_spawned then
                        ...
                        local corpse = SpawnPrefab("hh_corpse_beru")
                        if corpse then
    ...
    State {
        ...
        ["events"] = {
            EventHandler("animover", function(inst)
                if inst["AnimState"]:AnimDone() then
                    inst["sg"]:GoToState("3_times_idle")
                end
            end),
        },
        ["onexit"] = function(inst)
    ...
    State {
        ...
        ["onenter"] = function(inst)
            inst.Physics:Stop()
            inst.sg.statemem.loops = 3             -- Thiết lập số lần muốn phát hoạt ảnh (3 lần)
            ...
            EventHandler("animover", function(inst)
                if inst.AnimState:AnimDone() then
                    if inst.sg.statemem.loops > 1 then                     -- Kiểm tra nếu vẫn còn lượt lặp
                        ...
                        inst.sg:GoToState("attack_jump_pre")  -- Đã hoàn thành đủ 3 lần hoạt ảnh idle! Chuyển sang đòn attack2
                    end
                end
            end),
        },
    ...
    State {
        ...
        ["timeline"] = {
            FrameEvent(15, function(inst)
                ...
                then
                    inst["components"]["hh_buff"]:AddBuff(BUFF_NAME[random_num], 5)
                end
            end),
        },
    ...
    State {
        -- Gõ giáp háng khống chế diện rộng (Taunt CC)
        ["name"] = "pig_control",
...
return StateGraph("hh_beru_dungeon", hh_states, hh_events, "idle")