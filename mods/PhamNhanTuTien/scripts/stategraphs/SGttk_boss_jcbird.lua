...
local events =
{
    ...
    EventHandler("minhealth", function(inst)
        if inst.qianghua and inst:HasTag("spitter") and inst.components.health.minhealth > 0 then
            inst.sg:GoToState("xiumian")
        end
	end),
...
local states=
{
    ...
    State{
        ...
        timeline =
        {
            TimeEvent(0.82, function(inst)
                inst.sg.statemem.fx = SpawnAt("ttk_boss_jfsn_fire",inst)
                inst.sg.statemem.fx:SetChongSheng()
            end),
            TimeEvent(7.82, function(inst)
        ...
        {
            EventHandler("animover", function(inst)
                if inst.AnimState:AnimDone() and inst.AnimState:IsCurrentAnimation("sleep_pst") then
                    inst.sg:GoToState("idle")
                end
            end),
        },
...
return StateGraph("ttk_boss_jcbird", states, events, "idle", actionhandlers)
