...
local states=
{
    ...
    State{
        ...
        onenter = function(inst)
			inst.Physics:Stop()
			local care_speed = math.max(1, inst._hh_care_speed_mult or 1)
			inst.AnimState:SetDeltaTimeMultiplier(care_speed)
			inst.AnimState:PlayAnimation("plant_dance_pre", false)
			inst.AnimState:PushAnimation("plant_dance_loop", true)
			StartFlap(inst)
			inst.sg.statemem.care_task = inst:DoTaskInTime(77 * FRAMES / care_speed, function(inst)
				inst.sg.statemem.care_task = nil
                if inst:PerformBufferedAction() and inst.SayHHPlantLine ~= nil then
                    inst:SayHHPlantLine()
                    if inst.components.hh_shadow_unit ~= nil and inst.components.hh_shadow_unit:HasTalent("royal_gardener") then
                        local x, y, z = inst.Transform:GetWorldPosition()
                        local ents = TheSim:FindEntities(x, y, z, 8, {"tendable_farmplant"})
                        local extra_tended = 0
                        for _, v in ipairs(ents) do
                            if v.components.farmplanttendable ~= nil then
                                v.components.farmplanttendable:TendTo(inst)
                                extra_tended = extra_tended + 1
                                if extra_tended >= 2 then break end
                            end
                        end
                    end
                end
		...
		onexit = function(inst)
			if inst.sg.statemem.care_task ~= nil then
				inst.sg.statemem.care_task:Cancel()
				inst.sg.statemem.care_task = nil
			end
...
-- This custom flying graph owns the companion's Talker component and must
-- explicitly consume the vanilla ontalk/donetalking events.
...
return StateGraph("hh_fruitfly_shadow", states, events, "taunt", actionhandlers)
