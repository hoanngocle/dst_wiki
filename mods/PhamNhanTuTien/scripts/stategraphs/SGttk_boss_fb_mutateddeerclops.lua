...
local function DoIceLanceAOE(inst, pt, targets)
	...
	local ents =  XD_GetDamageTargets(pt.x, 0, pt.z, ICE_LANCE_RADIUS)
	for i, v in ipairs(ents) do
		if not targets[v] and v:IsValid() and not v:IsInLimbo() and
			not (v.components.health ~= nil and v.components.health:IsDead()) and
			XD_CanAttackTrget(inst,v)
		then
...
return StateGraph("ttk_boss_fb_mutateddeerclops", states, events, "corpse_mutate_pre", actionhandlers)
