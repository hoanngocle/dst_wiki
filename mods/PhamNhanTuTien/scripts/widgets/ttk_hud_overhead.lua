...
local Bar = Class(Widget, function(self, proxy)
    ...
    self.proxy = proxy
    self.bg = self:AddChild(Image("images/ttk_hud_overhead.xml", "dycghb_round_22.tex"))
    self.bg:SetSize(132, 18)
    self.bg:SetTint(0.08, 0.08, 0.08, 0.85)
    self.fill = self:AddChild(Image("images/ttk_hud_overhead.xml", "dycghb_round_22.tex"))
...
function Bar:OnUpdate()
    ...
    if TUNING.TTK_HUD.HIDE_BOSS_OVERHEAD and epic_displayed then
        self:Hide()
        return
    end
    ...
    self.text:SetString(TUNING.TTK_HUD.SHOW_VALUES and string.format("%d/%d", current, maximum) or "")
