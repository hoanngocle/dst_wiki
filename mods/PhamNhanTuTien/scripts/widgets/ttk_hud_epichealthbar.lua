...
local PersistentData = require "util/ttk_hud_persistentdata"
...
local function GetScissor()
	local size = TUNING.TTK_HUD_EPIC.FRAME_COLOUR[4] < 1 and -0.5 or 10
...
function MeterOverlay:TintTo(start, dest, duration, whendone, delay)
	self:CancelTintTo()
	self:SetTint(start.r, start.g, start.b, start.a)
	if delay == nil then
...
function MeterResist:ShowResist(tint, resist)
	...
	self.img:TintTo(TTK_HUDCore.Tint(tint, self.alpha), TTK_HUDCore.Tint(tint, 0), METER_BURST_TIME, nil, self.delay)
	self:Show()
end
...
function MeterDrops:ShowDrops(wet, instant, tint)
	...
	elseif wet then
		self:Show()
		self:TintTo(TTK_HUDCore.Tint(self.tint), TTK_HUDCore.Tint(tint, 1), METER_TINT_TIME)
	elseif self.shown then
		self:TintTo(TTK_HUDCore.Tint(self.tint), TTK_HUDCore.Tint(self.hiddentint), METER_TINT_TIME, function() self:Hide() end)
	end
end
...
local PopupNumber = Class(Widget, function(self, value, damaged, data)
	...
	if data.stimuli == "electric" then
		...
		self.colour2 = self:AddColour(TUNING.TTK_HUD_EPIC.ELECTRIC_ADDCOLOUR2)
		self.colour3 = self:AddColour(TUNING.TTK_HUD_EPIC.ELECTRIC_ADDCOLOUR3)
		self.colour = data.wet and self:AddColour(TUNING.TTK_HUD_EPIC.ELECTRIC_ADDCOLOUR1) or self.colour2
	end
...
function PopupNumber:GetSize(value)
	if self.pos then
		return TTK_HUDCore.Remap(value, 100, 400, 32, 40)
	else
...
function PopupNumber:FastForward()
	if not self.decay then
		...
		self.text:TintTo(TTK_HUDCore.Tint(self.text.colour), TTK_HUDCore.Tint(self.colour, 0), 0.2, function() self:Kill() end)
	end
end
...
local MiniButton = Class(ImageButton, function(self, icon, red)
	...
	if red then
		...
		self:SetImageNormalColour(TUNING.TTK_HUD_EPIC.BUTTON_COLOUR)
		self:SetImageFocusColour(TUNING.TTK_HUD_EPIC.BUTTON_COLOUR)
...
local ConfigurationScreen = Class(ModConfigurationScreen, function(self, widget)
	...
	self.inst:ListenForEvent("success", function(inst, settings)
		for i, v in ipairs(settings) do
			if TUNING.TTK_HUD_EPIC[v.name] ~= nil then
				TUNING.TTK_HUD_EPIC[v.name] = v.saved
			end
		end
...
function ConfigurationScreen:MakeServerReadOnly()
	for _, option in ipairs(self.config or {}) do
		if not option.client and #option.options > 1 then
			local data = TUNING.TTK_HUD_EPIC[option.name]
			if data == nil then
				data = option.default
			end
...
local function onwet(self, wet, oldwet)
	if wet ~= nil and wet ~= oldwet then
		local wet = wet and TUNING.TTK_HUD_EPIC.WETNESS_METER
...
local function onbuild(self, build, oldbuild)
	if build ~= nil and build ~= oldbuild then
		local theme = self:GetTuningValue("THEMES", self.target) or TUNING.TTK_HUD_EPIC.METER_COLOUR
		...
		if brightness > TUNING.TTK_HUD_EPIC.DARK_THRESH then
			self.bgtint = TUNING.TTK_HUD_EPIC.BACKGROUND_COLOUR1
			self.resisttint = theme

			if brightness >= TUNING.TTK_HUD_EPIC.POPUP_BRIGHTNESS then
				self.popuptint = theme
			else
				local mult = TUNING.TTK_HUD_EPIC.POPUP_BRIGHTNESS / brightness
				self.popuptint = {}
			...
			local damagetint = TUNING.TTK_HUD_EPIC.DAMAGE_COLOUR1
			if huediff(theme, damagetint) < TUNING.TTK_HUD_EPIC.HUE_THRESH then
				damagetint = TUNING.TTK_HUD_EPIC.DAMAGE_COLOUR2
			end
			self.damagetint = damagetint
		else
			self.bgtint = TUNING.TTK_HUD_EPIC.BACKGROUND_COLOUR2
			self.resisttint = TUNING.TTK_HUD_EPIC.DAMAGE_COLOUR1
			self.popuptint = TUNING.TTK_HUD_EPIC.DAMAGE_COLOUR1
			self.damagetint = TUNING.TTK_HUD_EPIC.DAMAGE_COLOUR1
		end
		...
		if oldbuild == nil then
			...
			self.meter:TintTo(TTK_HUDCore.Tint(self.meter.tint), TTK_HUDCore.Tint(self.metertint), METER_TINT_TIME)
			self.meter_bg:TintTo(TTK_HUDCore.Tint(self.meter_bg.tint), TTK_HUDCore.Tint(self.bgtint), METER_TINT_TIME)
			self.meter_damage:TintTo(TTK_HUDCore.Tint(self.meter_damage.tint), TTK_HUDCore.Tint(self.damagetint), METER_TINT_TIME)
			self:RefreshDrops()
		end
	end
end
...
local function oncurrenthealth(self, currenthealth, oldcurrenthealth)
	if currenthealth ~= nil and currenthealth ~= oldcurrenthealth then
		...
		if oldcurrenthealth ~= nil and TUNING.TTK_HUD_EPIC.DAMAGE_NUMBERS then
...
local function OnEpicTargetResisted(self, data)
	if self:TargetIs(data.target) and TUNING.TTK_HUD_EPIC.DAMAGE_RESISTANCE then
		self.meter_resist:ShowResist(self.resisttint, data.resist)
	end
end

local function OnGlobalPopupNumber(self, data)
	if not TUNING.TTK_HUD_EPIC.DAMAGE_NUMBERS then
		return
...
local EpicHealthbar = Class(Widget, function(self, owner, modinfo, modname)
	...
	self.data = PersistentData("pham_nhan_combat_hud_epic")
	self.timers = {}
	...
	self.meter_fg = self.barroot:AddChild(Image("images/ttk_hud_epichealthbar.xml", "meter_fg.tex"))
	self.meter_fg:SetSize(METER_WIDTH, METER_HEIGHT)

	self.frame = self.barroot:AddChild(Image("images/ttk_hud_epichealthbar.xml", "frame.tex"))
	self.frame:SetPosition(0, -2)
	self.frame:SetTint(unpack(TUNING.TTK_HUD_EPIC.FRAME_COLOUR))
	...
	if TUNING.TTK_HUD_EPIC.CAPTURE then
		self:MakeCaptureMode()
	elseif TUNING.TTK_HUD_EPIC.GLOBAL_NUMBERS then
		self.popuproot:Hide()
...
function EpicHealthbar:MakeCaptureMode()
	...
	local SavedCaptures = PersistentData("pham_nhan_combat_hud_epic_captures", true)
	...
	addsetter(self, "timeleft", function(self, timeleft)
		if timeleft ~= nil then
			if capture == nil then
				...
				self.captureicon:TintTo(TTK_HUDCore.Tint(WHITE, 5), TTK_HUDCore.Tint(WHITE, 0), 3, function() self.captureicon:Hide() end)
	...
	if EpicHealthbar._simreset ~= true then
		...
		TTK_HUDCore.Before(_G, "SimReset", function(params)
			local widget = TTK_HUDCore.Browse(ThePlayer, "HUD", "controls", "ttk_hud_epichealthbar")
			if widget ~= nil then
				widget:StopCapture()
			end
		end)
	...
	TTK_HUDCore.Sequence(self.meter_damage, "IsSuspended", function(suspended)
...
function EpicHealthbar:LoadConfigurationOptions()
	self:SetPosition(TUNING.TTK_HUD_EPIC.HORIZONTAL_OFFSET, 0)
	self:RebuildPhases()
	self:RefreshDrops()
	self:OnShow()
end
...
function EpicHealthbar:RebuildPhases()
	...
	if self.target ~= nil and TUNING.TTK_HUD_EPIC.FRAME_PHASES then
...
function EpicHealthbar:GetEffectTint(damaged, override)
	return (not damaged and TUNING.TTK_HUD_EPIC.HEAL_COLOUR)
		or (self.stimuli == "fire" and TUNING.TTK_HUD_EPIC.FIRE_COLOUR)
		or (override or TUNING.TTK_HUD_EPIC.DAMAGE_COLOUR1)
end
...
function EpicHealthbar:ShowBurst(start, dest)
	...
	self.meter_burst:TintTo(TTK_HUDCore.Tint(tint, 0.8), TTK_HUDCore.Tint(tint, 0), METER_BURST_TIME, function() self.meter_burst:Hide() end)
...
function EpicHealthbar:IsEpic()
	return not TUNING.TTK_HUD_EPIC.GLOBAL or TUNING.TTK_HUD_EPIC.TAG == "epic"
end
	...
	if not self:TimerExists("nodanger") and not self.dangerdisabled and self:IsEpic() or force then
		...
		if not self:IsPlayingMusic(target) then
			if self._startdanger ~= false and (self.danger or self:IsNear(target, DISENGAGED_DIST)) then
				if self._startdanger == nil then
					for i, fn in ipairs(TTK_HUDCore.Browse(TheWorld, "event_listening", "attacked", self.owner) or {}) do
						if TTK_HUDCore.GetUpvalue(fn, "StartDanger") then
		...
		elseif not self:TimerExists("notrigger") or force then
			...
			elseif value ~= true then
				if value == nil then
					value = TTK_HUDCore.GetUpvalue(Prefabs[target.prefab].fn, "PushMusic")
...
function EpicHealthbar:GetIsSpectator()
	...
	return self.owner:HasAnyTag("hiding", "sitting_on_chair")
...
function EpicHealthbar:RefreshMenu()
	...
	elseif TheFocalPoint.components.focalpoint:IsFocusBlocked(self.inst) or TheFocalPoint ~= TheCamera.target then
		...
		self.camera:SetTooltip(self:GetLocalizedString("Not Available!", "CAMERA_BUTTON", 3))
	elseif not self.spectator and self.target ~= nil and not self:IsNear(self.target, TUNING.TTK_HUD_EPIC.CAMERA_FOCUS_MIN) then
		self.camera:Enable(false)
		self.camera:SetTooltip(self:GetLocalizedString("Too Far!", "CAMERA_BUTTON", 2))
	else
		self.camera:Enable(true)
		self.camera:SetTooltip(self:GetLocalizedString("Toggle Combat Camera", "CAMERA_BUTTON", 1))
	end
end
...
function EpicHealthbar:FocusCamera(enable)
	if TheFocalPoint == nil then
		return
	elseif enable and self:IsEpic() and not TUNING.TTK_HUD_EPIC.CAPTURE then
		TheFocalPoint.components.focalpoint:StartFocusSource(self.inst, "FIXED", self.owner, TUNING.TTK_HUD_EPIC.CAMERA_FOCUS_MIN, TUNING.TTK_HUD_EPIC.CAMERA_FOCUS_MAX, TUNING.TTK_HUD_EPIC.CAMERA_PRIORITY, { ActiveFn = function(params) self.camerafocus = setmetatable({}, { __mode = "k" }); params.nofocus, params.count = 0, 0 end, UpdateFn = function(...) self:UpdateFocus(...) end })
	else
	...
	if not params.nofocus then
		...
		if count > 0 then
			...
			elseif dist < params.minrange then
				percent = TTK_HUDCore.Remap(dist, params.minrange / 2, params.minrange, 0.5, 0)
			end
...
function EpicHealthbar:GetTuningValue(type, target)
	...
	local value = FunctionOrValue(TUNING.TTK_HUD_EPIC[type][key], target)
	if value ~= nil then
		return value
	elseif type ~= "PHASES" then
		for key, value in pairs(TUNING.TTK_HUD_EPIC[type]) do
...
function EpicHealthbar:GetLocalizedString(default, ...)
	return TTK_HUDCore.Browse(self.modinfo, "STRINGS", ...) or default
end
...
function EpicHealthbar:IsBusy(target)
	...
	elseif target:HasTag("minhealth1") then
		return not target:HasTag("hostile") --daywalker, sharkboi
			or target:HasTag("noclick") --alterguardian
	elseif target:HasTag("attack") or self:IsInIntro(target) then
		return false
	elseif target:HasTag("flight") then --dragonfly, beequeen
		return target:HasTag("busy")
	elseif target:HasTag("noattack") then
		return target.epichealth.invincible --intro/outro animation
			or target:HasTag("noclick") --shadow chess
			or not target:HasTag("locomotor") --antlion/crabking??
	end
	return target.replica.inventoryitem ~= nil and target:HasTag("inlimbo")
end

function EpicHealthbar:IsValidTarget(target)
	return self.targets[target]
		and target:HasTag(TUNING.TTK_HUD_EPIC.TAG)
		...
		and not (target.replica.follower ~= nil and target.replica.follower:GetLeader() == self.owner)
