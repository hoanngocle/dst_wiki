...
local function makerock(rocktype)
    ...
    return Prefab("hh_arena_lava_pond_rock"..rocktype, fn, rock_assets)
end
...
local function SpawnRocks(inst)
    ...
    for i, v in ipairs(inst.rocks) do
        if type(v.rocktype) == "string" and type(v.offset) == "table" and #v.offset == 3 then
            local rock = SpawnPrefab("hh_arena_lava_pond_rock"..v.rocktype)
            if rock ~= nil then
...
local function OnCollide(inst, other)
    if other ~= nil and
        ...
        if other:HasTag("player") then
            is_target = true
        elseif other.components.follower and other.components.follower.leader and other.components.follower.leader:HasTag("player") then
            is_target = true
        end
...
local function DoDamage(inst)
    ...
    for i, v in ipairs(ents) do
        if v:IsValid() and v.components.health and not v.components.health:IsDead() then
            ...
            if is_target then
                if v.components.burnable and not v.components.burnable:IsBurning() then
...
local function fn()
    ...
    -- Thay vì dùng propagator để đốt cháy mọi thứ, ta dùng hàm DoPeriodicTask
    -- để giả lập lại propagator nhưng có chọn lọc mục tiêu
    inst:DoPeriodicTask(1, DoDamage)
...
local ret = { makerock("") }
local prefabs = { "hh_arena_lava_pond_rock" }
for i = 2, NUM_ROCK_TYPES do
    table.insert(ret, makerock(tostring(i)))
    table.insert(prefabs, "hh_arena_lava_pond_rock"..tostring(i))
end
table.insert(ret, Prefab("hh_arena_lava_pond", fn, assets, prefabs))
prefabs = nil
return unpack(ret)
