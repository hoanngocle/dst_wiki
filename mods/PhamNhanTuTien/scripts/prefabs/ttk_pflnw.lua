...
local assets =
{
    Asset("ANIM", "anim/ttk_pflnw.zip"),
	Asset("ATLAS", "images/inventoryimages/ttk_pflnw.xml"),
    Asset("IMAGE", "images/inventoryimages/ttk_pflnw.tex"),
}

local prefabs = { "ttk_beefalo", "collapse_small", "beefalowool", "horn",
...
local function OnDay(inst)
    if inst.components.childspawner:IsFull() then
		...
		if inst.day_count >= 3 then
			...
			if left > 1 then
				for k = 1,8 do
					inst.components.inventory:GiveItem(SpawnPrefab('beefalowool'))
				end
				if math.random() < 0.5 then
					inst.components.inventory:GiveItem(SpawnPrefab('horn'))
				end
			end
		end
    end
...
local function fn()
    ...
    inst.AnimState:SetBank("xd_pflnw")
    inst.AnimState:SetBuild("xd_pflnw")
    inst.AnimState:PlayAnimation("idle")

	MakeObstaclePhysics(inst, .8)

    inst.MiniMapEntity:SetIcon("ttk_pflnw.tex")
    ...
    inst.components.childspawner.childname = "ttk_beefalo"
...
return  Prefab("ttk_pflnw", fn, assets, prefabs),
    MakePlacer("ttk_pflnw_placer", "xd_pflnw", "xd_pflnw", "idle")
