local assets =
{
    ...
    Asset("ANIM", "anim/ttk_spider.zip"),
...
local proassets =
{
    Asset("ANIM", "anim/ttk_spider_pro.zip"),
    Asset("ANIM", "anim/ttk_slow_buff_ent.zip"),
}
...
local prefabs =
{
    ...
    "ttk_spider_pro", "ttk_spider_puff_front", "ttk_spider_puff_back",
    "ttk_slow_buff", "ttk_slow_buff_ent",
}

local brain = require "brains/ttk_spiderbrain"
...
local SPIDERDEN_TAGS = {"ttk_spiderden"}
...
local function OnAttacked(inst, data)
    ...
    if not houseutil.CanAttackTarget(inst, data.attacker) then
        return
    end
    inst.components.combat:SetTarget(data.attacker)
    inst.components.combat:ShareTarget(data.attacker, 30, function(dude)
        local should_share = dude:HasTag("ttk_spider")
...
local function DoDamage(inst,target,damage)
    if not houseutil.CanAttackTarget(inst, target) then
        return
    end
...
local function OnHitOther(inst, data)
	...
	if inst.isshadow and data.target ~= nil and data.target.components.ttk_poisonable ~= nil then
        data.target.components.ttk_poisonable:Poison()
    end
end
...
local function create_common(bank, build, tag, common_init, extra_data)
    ...
    inst:SetStateGraph((extra_data and extra_data.sg) or "SGttk_spider")
...
local function create_spider()
    local inst = create_common("spider", "xd_spider")
...
local function OnHit_Lunar(inst, attacker, target)
    if target and target:IsValid() and target:HasTag("player") then
        if not IsEntityDeadOrGhost(target) then
            target:AddDebuff("ttk_slow_buff","ttk_slow_buff")
        end
    end
    inst:Remove()
end
...
local function profn()
	...
	inst.AnimState:SetBank("xd_spider_pro")
	inst.AnimState:SetBuild("xd_spider_pro")
	inst.AnimState:PlayAnimation("idle")
...
local function bufffn()
    ...
    inst.AnimState:SetBank("xd_slow_buff_ent")
    inst.AnimState:SetBuild("xd_slow_buff_ent")
    inst.AnimState:PlayAnimation("idle")
...
return Prefab("ttk_spider", create_spider, assets, prefabs),
    Prefab("ttk_spider_pro", profn, proassets),
    Prefab("ttk_slow_buff_ent", bufffn, proassets)
