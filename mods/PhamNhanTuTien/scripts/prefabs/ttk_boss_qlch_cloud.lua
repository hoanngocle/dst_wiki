...
local assets =
{
    ...
    Asset("ANIM", Boss.ArtPath("anim/xd_qlch_cloud.zip")),
    Asset("ANIM", Boss.ArtPath("anim/xd_qlch_cloud_base.zip")),
}
local prefabs =
{
    "ttk_boss_qlch_cloud_overlay",
}
local FADE_FRAMES = 5
...
local function SpawnOverlayFX(inst, i, set, isnew)
    ...
    local fx = SpawnPrefab("ttk_boss_qlch_cloud_overlay")
...
local function CreateBase(isnew)
    ...
    inst.AnimState:SetBuild(Boss.Art("xd_qlch_cloud_base"))
...
local function OnTimerDone(inst, data)
    if data.name == "disperse" then
        ...
        local notags = { "FX", "NOCLICK", "INLIMBO", "qlch"}
        local ents = TheSim:FindEntities(x,y,z,3.5,{"_health","_combat"},notags)
...
local function DoAreaSpoil(inst)
    local x, y, z = inst.Transform:GetWorldPosition()
    local ents = TheSim:FindEntities(x, y, z, 3.5, nil, { "playerghost", "FX", "DECOR", "INLIMBO" })
    for i, v in ipairs(ents) do
        if not inst.targets[v] and
...
local function fn()
    ...
    inst.AnimState:SetBuild(Boss.Art("xd_qlch_cloud"))
    ...
    inst._state = net_tinybyte(inst.GUID, "ttk_boss_qlch_cloud._state", "statedirty")
    inst._fade = net_smallbyte(inst.GUID, "ttk_boss_qlch_cloud._fade", "fadedirty")
    inst._fadetask = inst:DoPeriodicTask(FRAMES, OnUpdateFade)
...
return Prefab("ttk_boss_qlch_cloud", fn, assets, prefabs),
    Prefab("ttk_boss_qlch_cloud_overlay", overlayfn, assets)
