...
if not TheNet:IsDedicated() then
	require("widgets/ttk_hud_epichealthbar").targets = AllEpicTargets
end
...
local function OnAttacked(parent, data)
	if data ~= nil and data.original_damage ~= nil and data.original_damage > 0 and not data.redirected then
		local stimuli = data.stimuli or TTK_HUDCore.Browse(data.weapon, "components", "weapon", "stimuli")
...
local function fn()
	...
	TTK_HUDCore.Replica(inst, OnEntityReplicated)
...
return Prefab("ttk_hud_epichealth_proxy", fn)