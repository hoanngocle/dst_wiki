...
local function onextinguish(inst, addLootItems)
	...
	local bonus = ttk_vhth_dropLootHeatStar
...
local function updatefuelrate(inst)
	inst.components.fueled.rate = TheWorld.state.israining and ttk_vhth_burnRateHeatStar or ttk_vhth_burnRateHeatStar
end
...
local function fn()
	...
	bonus = ttk_vhth_structureSizeHeatStar
	if bonus == "nooverride" then
	...
	bonus = ttk_vhth_maxFuelHeatStar
	if bonus == "nooverride" then
	...
	bonus = ttk_vhth_efficiencyHeatStar
	if bonus == "nooverride" then
	...
	inst.components.fueled:SetSectionCallback(
		function(section, oldSection)
			...
			if section == 0 then
				...
				inst:ListenForEvent(
					...
					function()
						...
						inst.SoundEmitter:PlaySound("dontstarve/common/fireAddFuel")
						addLootItems = 1 -- initialise loot on build
					end
				)
			end

			bonus = ttk_vhth_sanityBoostHeatStar
			if bonus == "nooverride" then
			...
			elseif section > 3 and section <= 8 then
				inst.components.sanityaura.aura = TUNING.SANITYAURA_MED * bonus
			elseif section > 0 and section <= 3 then
	...
	--
	bonus = ttk_vhth_startFuelHeatStar
	if bonus == "nooverride" then
...
return Prefab("common/objects/heat_star", fn, assets, prefabs), MakePlacer(
	"common/heat_star_placer",
	"heat_star",
	"heat_star",
	"idle"
)
