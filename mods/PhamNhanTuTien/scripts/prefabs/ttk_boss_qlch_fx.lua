...
local function deer_ice_circle_common_postinit(inst)
    ...
    inst._rad = net_float(inst.GUID, "qlch_ice_circle._rad")
...
local function MakeFX(name, data)
    ...
    if data.bank then
        table.insert(assets, Asset("ANIM", Boss.ArtPath("anim/"..data.bank..".zip")))
    end
    local prefabs = {}
    ...
    local function fn()
        ...
        inst.AnimState:SetBank(Boss.Art(data.bank or name))
        ...
        if data.light then
            ...
            inst._fade = net_smallbyte(inst.GUID, "qlch_fx._fade", "fadedirty")
            inst._fadetask = inst:DoPeriodicTask(FRAMES, OnUpdateFade)
        end
        inst:AddTag("FX")
