...
local function OnUpdateHitbox(inst)
	...
	local ents = XD_GetDamageTargets(x, y, z,radius + AOE_RANGE_PADDING)
	for i, v in ipairs(ents) do
		if v ~= inst.attacker and v:IsValid() and not v:IsInLimbo() and not (v.components.health and v.components.health:IsDead()) then
			if not inst.attacker:HasTag("player") or not inst.attacker.components.combat:IsAlly(v) and XD_CanAttackTrget(inst.attacker,v) then
...
local function fn()
	...
	inst:AddTag("FX")
	inst:AddTag("NOCLICK")
	inst.brightness = net_tinybyte(inst.GUID, "ttk_boss_fb_warg_mutated_breath_fx.brightness", "brightnessdirty")
	inst.brightness:set(7)
	OnBrightnessDirty(inst)
...
local function emberfn()
	...
	inst:AddTag("FX")
	inst:AddTag("NOCLICK")
	inst.fade = net_smallbyte(inst.GUID, "ttk_boss_fb_warg_mutated_ember_fx.fade", "fadedirty")
	inst.fizzle = net_smallbyte(inst.GUID, "ttk_boss_fb_warg_mutated_ember_fx.fizzle")
	inst:AddComponent("updatelooper")
	inst.entity:SetPristine()
	if not TheWorld.ismastersim then
...
return Prefab("ttk_boss_fb_warg_mutated_breath_fx", fn, assets, prefabs),
	Prefab("ttk_boss_fb_warg_mutated_ember_fx", emberfn, assets)
