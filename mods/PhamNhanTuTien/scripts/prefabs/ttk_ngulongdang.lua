local assets =
{
    ...
    Asset("ANIM", "anim/ttk_ngulongdang.zip"),
    Asset("ANIM", "anim/redlantern_water.zip"),
    Asset("ANIM", "anim/swap_ttk_ngulongdang.zip"),
    Asset("ATLAS", "images/inventoryimages/ttk_ngulongdang.xml"),
    Asset("IMAGE", "images/inventoryimages/ttk_ngulongdang.tex"),
}

local prefabs =
{
    "ttk_ngulongdanglight",
    "ttk_ngulongdangbody",
}
...
local function turnon(inst)
    if not inst.components.fueled:IsEmpty() then
        ...
        if inst._light == nil then
            inst._light = SpawnPrefab("ttk_ngulongdanglight")
...
local function ToggleOverrideSymbols(inst, owner)
    local skinbuild = inst:GetSkinBuild() or "xd_sudaji_redlantern"
    if owner.sg ~= nil and (owner.sg:HasStateTag("nodangle")
        ...
        owner.AnimState:OverrideSymbol("swap_object", "swap_" .. skinbuild, "swap_redlantern")
        if not inst.components.fueled:IsEmpty() then
            owner.AnimState:Show("LANTERN_OVERLAY")
        end
        inst._body:Hide()
    else
        owner.AnimState:OverrideSymbol("swap_object", "swap_" .. skinbuild, "swap_redlantern_stick")
...
local function onequip(inst, owner)
    ...
    owner.AnimState:OverrideSymbol("lantern_overlay", "swap_" .. skinbuild, "redlantern_overlay")

    if inst._body ~= nil then
        inst._body:Remove()
    end
    inst._body = SpawnPrefab("ttk_ngulongdangbody")
    inst._body._lantern = inst
    inst._body.AnimState:SetBuild(skinbuild)
    ...
    inst._body.Follower:FollowSymbol(owner.GUID, "swap_object", 60, -152, 0)
...
local function OnAddFuelItem(inst, item, fuelvalue, doer)
    ...
    if not inst.components.inventoryitem.owner or inst.components.equippable:IsEquipped() then
...
local function fn()
    ...
    inst.AnimState:SetBank("xd_sudaji_redlantern")
    inst.AnimState:SetBuild("xd_sudaji_redlantern")
    ...
    inst.components.inventoryitem.atlasname = "images/inventoryimages/ttk_ngulongdang.xml"
    ...
    inst.components.fueled.fueltype = "TTK_NGULONGDANG"
...
return Prefab("ttk_ngulongdang", fn, assets, prefabs),
    Prefab("ttk_ngulongdanglight", lanternlightfn),
    Prefab("ttk_ngulongdangbody", lanternbodyfn)
