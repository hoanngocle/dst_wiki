...
local assets =
{
    Asset("ANIM", "anim/ttk_ftys.zip"),
	Asset("ATLAS", "images/inventoryimages/ttk_ftys.xml"),
    Asset("IMAGE", "images/inventoryimages/ttk_ftys.tex"), 
}

local prefabs = { "ttk_lightninggoat", "collapse_small", "goatmilk", "lightninggoathorn",
...
local function OnDay(inst)
    if inst.components.childspawner:IsFull() then
        ...
        if left > 1 then
            inst.components.inventory:GiveItem(SpawnPrefab('goatmilk'))
            if math.random() < 0.25 then
                inst.components.inventory:GiveItem(SpawnPrefab('lightninggoathorn'))
            end
        end
    end
end
...
local function fn()
    ...
    inst.AnimState:SetBank("xd_ftys")
    inst.AnimState:SetBuild("xd_ftys")
    inst.AnimState:PlayAnimation("idle")

	MakeObstaclePhysics(inst, .8)

    inst.MiniMapEntity:SetIcon("ttk_ftys.tex")
    ...
    inst.components.childspawner.childname = "ttk_lightninggoat"
...
return  Prefab("ttk_ftys", fn, assets, prefabs),
    MakePlacer("ttk_ftys_placer", "xd_ftys", "xd_ftys", "idle")
