...
local Travelable = Class(function(self, inst)
    ...
    self._infos = net_string(inst.GUID, "ttt_travelable._infos")

    self.screen = nil
    self.opentask = nil

    if TheWorld.ismastersim then
        self.classified = SpawnPrefab("ttt_travelable_classified")
        self.classified.entity:SetParent(inst.entity)
    else
        if self.classified == nil and inst.ttt_travelable_classified ~= nil then
            self.classified = inst.ttt_travelable_classified
            inst.ttt_travelable_classified.OnRemoveEntity = nil
            inst.ttt_travelable_classified = nil
...
function Travelable:BeginTravel(traveller)
    if self.inst.components.ttt_travelable ~= nil then
        if self.opentask ~= nil then
            self.opentask:Cancel()
            self.opentask = nil
        end
        self.inst.components.ttt_travelable:BeginTravel(traveller)
...
function Travelable:Travel(traveller, index)
    if self.inst.components.ttt_travelable ~= nil then
        self.inst.components.ttt_travelable:Travel(traveller, index)
    elseif self.classified ~= nil and traveller == ThePlayer then
        SendModRPCToServer(MOD_RPC.NYX_TTT.Travel, self.inst, index)
    end
end
...
function Travelable:EndTravel()
    ...
    if self.inst.components.ttt_travelable ~= nil then
        self.inst.components.ttt_travelable:EndTravel()
    elseif self.screen ~= nil then
        if ThePlayer ~= nil and ThePlayer.HUD ~= nil then
            ThePlayer.HUD:CloseTTTTravelScreen()
...
function Travelable:SetTraveller(traveller)
    ...
    if self.inst.components.ttt_travelable == nil then
        -- Should only reach here during ttt_travelable construction
