...
local function HasPersonalOwner(item)
    local strengthen = item.components.wb_strengthen
    if strengthen ~= nil and type(strengthen.HasBuff) == "function" and strengthen:HasBuff("bind") then return true end
...
local function pickup(inst,self)
    ...
    for i, v in ipairs(ents) do
        if v:IsValid()  and v.components.inventoryitem ~= nil and
            ...
            if fx ~= nil then fx.Transform:SetPosition(v.Transform:GetWorldPosition()) end
