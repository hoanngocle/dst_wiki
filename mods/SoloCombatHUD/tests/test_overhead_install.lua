...
local game = {
    ...
    TUNING = { SCHUD = { OVERHEAD_BAR = true } },
...
local prefab_source = assert(io.open("scripts/prefabs/schud_overhead_proxy.lua", "r")):read("*all")
...
local widget_source = assert(io.open("scripts/widgets/schud_overhead.lua", "r")):read("*all")
