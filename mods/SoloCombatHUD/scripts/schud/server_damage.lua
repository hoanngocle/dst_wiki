local Resolver = require("schud/resolver")
local Bridge = require("schud/solo_bridge")
...
local resolver = Resolver.new(function(hit)
    ...
    SendModRPCToClient(CLIENT_MOD_RPC.schud.damage, player.userid,
        hit.victim, hit.amount, hit.kind, x, y, z)
    if TUNING ~= nil and TUNING.SCHUD ~= nil and TUNING.SCHUD.SHOW_OTHERS then
        for _, other in ipairs(AllPlayers) do
            if other ~= player and other.userid ~= nil and other:GetDistanceSqToPoint(x, y, z) <= 1225 then
                SendModRPCToClient(CLIENT_MOD_RPC.schud.damage, other.userid,
                    hit.victim, hit.amount, hit.kind, x, y, z)
            end
        end
    end
