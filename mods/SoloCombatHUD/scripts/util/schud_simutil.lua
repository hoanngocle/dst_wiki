function SCHUDCore.Tint(tint, alpha, raw)
	if raw then
...
function SCHUDCore.Alpha(alpha)
	return SCHUDCore.Tint(WHITE, alpha)
end

function SCHUDCore.FormatTimer(time)
	local bits = {}
...
function SCHUDCore.Load(assets)
...
function SCHUDCore.Replica(inst, fn)
	if TheWorld.ismastersim or inst.Network == nil then
		inst:DoTaskInTime(0, fn)
	else
		SCHUDCore.After(inst, "OnEntityReplicated", fn)
	end
end
