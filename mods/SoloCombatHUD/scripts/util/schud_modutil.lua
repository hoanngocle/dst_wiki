...
function GetPostInitFns(type, key)
	...
	return postinitfns[type] or SCHUDCore.Empty
end

function GetPostInitData(type)
	return postinitdata[type] or SCHUDCore.Empty
end
...
function AddModsPostInit(fn)
	if not HasPostInit("Mods") then
		SCHUDCore.After(_G, "TranslateStringTable", function() --this is called right after all mods
	...
	if not HasPostInit("Widget") then
		AddSimPostInit(function()
			for name, fns in pairs(GetPostInitFns("Widget")) do
				SCHUDCore.Init(require(name), function(...)
...
if not TheNet:IsDedicated() then
	...
	AddComponentPostInit("focalpoint", function(self, inst)
		...
		SCHUDCore.Branch(self, "Reset", function(Reset, self, ...)
		...
		SCHUDCore.Branch(self, "StopFocusSource", function(StopFocusSource, self, source, id, ...)
			if id ~= "FIXED" then
...
if IsInFrontEnd() then
	...
	function AddModsTabData(type, data)
		data.type = type
		data.onupdate = data.onupdate or SCHUDCore.Dummy
		...
		for key, data in pairs(extras) do
			...
			elseif data.type == "button" then
				...
				if extras.menu == nil or extras.menu.inst == nil then
					...
					extras.menu = { onupdate = SCHUDCore.Dummy }
	...
	local function PatchModsTab()
		...
		if mods_tab[key] == nil then
			SCHUDCore.After(mods_tab, "ShowModDetails", function(self)
...
for pre, pst in pairs({ ModRPC = "Server", ClientModRPC = "Client", ShardModRPC = "Shard" }) do
	...
	env[addfn] = function(namespace, name, fn)
		if fn == nil then
			namespace, name, fn = "SCHUDCore", namespace, name
		end
	...
	env[sendfn] = function(rpc, ...)
		if type(rpc) == "string" then
			rpc = _G[getfn]("SCHUDCore", rpc)
		end
		_G[sendfn](rpc, ...)
	end

	SCHUDCore[sendfn] = env[sendfn]
end