...
if not TheNet:IsDedicated() then
	require("widgets/schud_epichealthbar").targets = AllEpicTargets
end
...
local function OnAttacked(parent, data)
	if data ~= nil and data.original_damage ~= nil and data.original_damage > 0 and not data.redirected then
		local stimuli = data.stimuli or SCHUDCore.Browse(data.weapon, "components", "weapon", "stimuli")
...
local function fn()
	...
	SCHUDCore.Replica(inst, OnEntityReplicated)
...
return Prefab("schud_epichealth_proxy", fn)