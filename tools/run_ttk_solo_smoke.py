...
RUNTIME = ROOT / '.superpowers/ttk-solo-integration/server'
...
def main():
    ...
    port = 12194 if args.migration else 12193
    ...
    expected = 'TTK_SOLO_RELOAD_PASS' if args.reload else 'TTK_SOLO_CREATE_PASS'
    success = False
    with log_path.open('w', encoding='utf-8') as log:
        ...
        try:
            ...
            while process.poll() is None and time.monotonic() < deadline:
                ...
                if any(marker in lines for marker in ('LUA ERROR', 'MOD ERROR:', 'TTK_SOLO_FAIL',
                                                      'Server failed to start!', 'Unhandled exception')):
                    break
                if not sent and 'Telling Client our new session identifier:' in text:
                    script = (ROOT / 'mods/PhamNhanTuTien/tools/solo_integration_smoke.lua').read_text(encoding='utf-8')
