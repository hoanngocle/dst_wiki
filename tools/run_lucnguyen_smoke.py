"""Run the ranged weapon in a separate DST world; never touch a player's save."""
...
runtime = ROOT / '.superpowers/lucnguyen-audit'
...
args = [str(runtime / 'bin64/dontstarve_dedicated_server_nullrenderer_x64.exe'),
        ...
        '-cluster', 'Cluster_LucNguyen', '-shard', 'Master', '-offline', '-console', '-skip_update_server_mods']
out = runtime / 'lucnguyen_smoke.log'
success = False
    ...
    try:
        ...
        while process.poll() is None and time.time() < deadline:
            ...
            if not sent and 'Telling Client our new session identifier:' in text:
                script = (ROOT / 'mods/PhamNhanTuTien/tools/lucnguyen_smoke.lua').read_text(encoding='utf-8')
                ...
                sent = True
                print('Luc Nguyen smoke dispatched', flush=True)
            if 'TTK_LUCNGUYEN_SMOKE_FAIL' in lines or 'LUA ERROR' in lines:
                break
            if 'TTK_LUCNGUYEN_SMOKE_PASS' in lines:
                success = True
                break
    finally:
