  ...
  return (
    <>
      ...
      <span aria-hidden="true">
        ...
        <mark className="rounded-sm bg-nova-accent/15 px-0.5 text-inherit">
...
export function ItemResult({
  ...
  return (
    <article className="catalog-card h-full rounded-2xl border border-nova-border bg-nova-surface-soft p-4 shadow-[0_10px_28px_rgba(40,66,98,0.06)] transition duration-200 hover:-translate-y-0.5 hover:border-nova-border-strong hover:shadow-[0_14px_32px_rgba(40,66,98,0.1)] motion-reduce:transform-none motion-reduce:transition-none">
      <button
        type="button"
        disabled={!onSelectItem}
        aria-label={`Xem chi tiết ${item.name}`}
        onClick={() => onSelectItem?.(item)}
        className="block w-full cursor-pointer rounded-xl text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-nova-accent disabled:cursor-default"
      >
        <div className="grid grid-cols-[72px_minmax(0,1fr)] items-center gap-3.5">
          <GameSprite sprite={item.sprite} size={72} className="ring-1 ring-nova-border" />
          <div className="min-w-0">
            <div className="mb-1.5 flex flex-wrap items-center gap-1.5 text-[11px] font-semibold">
              <span className="rounded-full border border-nova-accent/30 bg-nova-accent/10 px-2 py-0.5 text-nova-accent">
                {sourceLabel}
              </span>
              <span
                aria-label={`Loại: ${category.label}`}
                className="inline-flex items-center gap-1 rounded-full border border-nova-border bg-nova-surface-raised px-2 py-0.5 text-nova-muted"
              >
            ...
            {realPrefab ? (
              ...
              <span className="mt-1 block text-xs font-medium text-nova-muted">Wiki item</span>
            )}
          </div>
        </div>

        {item.description ? (
          <p className="mt-3 line-clamp-2 text-sm leading-6 text-nova-muted">
      ...
      {item.recipe ? (
        <div className="mt-4 border-t border-nova-border pt-3">
          <p className="mb-2 text-xs font-semibold text-nova-muted">Công thức</p>
          <RecipeIngredients
            recipe={item.recipe}
            itemsById={itemsById}
