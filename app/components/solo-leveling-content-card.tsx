...
export function SoloLevelingContentCard({ entry, topic }: { entry: SoloLevelingEntry; topic: string }) {
  ...
  const prefabLine = entry.lines
    .find((line) => /^\s*-?\s*Prefab\s*:/i.test(line))
    ?.replace(/^\s*-?\s*Prefab\s*:\s*/i, "Prefab: ");
  const contentLines = entry.lines.filter((line) => !/^\s*-?\s*Prefab\s*:/i.test(line));
  const movedWikiItem = entry.source === "Wiki.txt";
  const itemLayout = topic === "crafting";
  const document = topic === "guide" || (!itemLayout && movedWikiItem) || entry.tables.length > 0;
  const recipe = entry.recipe;
  const recipes = entry.recipes ?? (recipe ? [recipe] : []);
  const facts = contentLines.filter((line) => /^(Mục tiêu:|Độ khó:|Rank:|Xu Hiệp Hội:|Thời hạn \(ngày\):|Vai trò:|Hoạt động nhận EXP:|\d+ EXP$)/.test(line));
  const details = contentLines.filter((line) => !facts.includes(line));
  const remaining = itemLayout || recipe ? contentLines : details.slice(details[0]?.startsWith("Kỹ năng theo cấp:") ? 0 : 1).filter((line) => !entry.talents || !line.startsWith("Kỹ năng theo cấp:"));
  return (
    <article id={entry.id} aria-labelledby={`${entry.id}-title`} className={`catalog-card flex min-w-0 scroll-mt-6 flex-col rounded-2xl border border-nova-border bg-nova-surface-soft p-4 sm:p-5 ${document ? "col-span-full" : ""}`}>
      <div className="flex items-center gap-3.5">
        ...
        <div className="min-w-0">
          {entry.category && entry.category !== "Wiki" && <p className="text-xs font-medium text-nova-accent">{entry.category}</p>}
          <h3 id={`${entry.id}-title`} className={`${entry.category && entry.category !== "Wiki" ? "mt-1 " : ""}text-lg leading-6 font-semibold tracking-[-0.02em] [overflow-wrap:anywhere]`}>{entry.title}</h3>
          {prefabLine && <code className="mt-1 block break-all text-[11px] leading-4 text-nova-faint">{prefabLine}</code>}
        </div>
      </div>
      {itemLayout ? <>
        <p className="mt-4 text-xs font-semibold uppercase tracking-wider text-nova-faint">Nguyên liệu</p>
        {recipes.length > 0 ? <div className="mt-2 space-y-3">
          {recipes.map((variant, index) => <div key={`${variant.prefab}:${index}`} className="rounded-xl border border-nova-border bg-nova-surface/60 p-3">
            {recipes.length > 1 && <p className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-nova-faint">Công thức {index + 1}</p>}
            <RecipeIngredients recipe={{ outputCount: variant.amount, ingredients: variant.ingredients.map((ingredient) => ({ id: ingredient.prefab, name: ingredient.name, amount: ingredient.amount, sprite: ingredient.sprite ?? null })) }} />
            <p className="mt-3 flex items-start gap-2 text-sm leading-6 text-nova-muted"><Hammer className="mt-1 shrink-0" size={18} aria-hidden="true" /><span>Chế tạo tại: {variant.station}</span></p>
          </div>)}
        </div> : <div className="mt-2 rounded-xl border border-nova-border bg-nova-surface/60 p-3"><RecipeIngredients recipe={null} /></div>}
      </> : <>
        {document ? <div className="mt-4"><EntryLines lines={contentLines} /></div> : <>
      ...
      {!document && <details className="mt-4 border-t border-nova-border pt-3 text-xs leading-6 text-nova-faint"><summary className="cursor-pointer rounded-md font-medium focus-visible:outline-2 focus-visible:outline-nova-accent">Xem đầy đủ thông tin</summary><div className="mt-3"><EntryLines lines={remaining} /></div>{!movedWikiItem && <p className="mt-3 break-all">Nguồn: {entry.source}</p>}</details>}
      {document && !movedWikiItem && <p className="mt-4 break-all border-t border-nova-border pt-3 text-xs leading-5 text-nova-faint">Nguồn: {entry.source}</p>}
    </article>
  );
}
