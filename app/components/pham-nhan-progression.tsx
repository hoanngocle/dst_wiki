...
const availabilityLabels: Record<string, string> = { implemented: "Đã có triển khai", partial: "Một phần", unavailable: "Chưa khả dụng" };
...
export function PhamNhanProgression() {
  ...
  return <div className="space-y-8">
    ...
    <DstPanel className="p-5 sm:p-6">
      ...
      <p className="mt-3 leading-7 text-nova-muted">{data.perks.length} kỹ năng và {data.perks.reduce((sum, perk) => sum + perk.maxCost, 0)} Star là tổng của toàn bộ danh mục, bao gồm nội dung chưa khả dụng. Hiện có {data.perks.filter((perk) => perk.availability.status === "implemented").length} kỹ năng đã có triển khai, {data.perks.filter((perk) => perk.availability.status === "partial").length} gói một phần và {data.perks.filter((perk) => perk.availability.status === "unavailable").length} kỹ năng/gói chưa khả dụng. Kiểm kê mã nguồn chưa thay thế kiểm chứng đầy đủ trong DST.</p>
      <p className="mt-2 break-words text-xs text-nova-muted">Nguồn tình trạng: {data.perkRuntimeSource}</p>
      <p className="mt-3 text-nova-muted">Giá kỹ năng nhiều bậc: bậc 1–10 giá 2, 11–15 giá 3, 16–20 giá 4, 21–25 giá 5 Star mỗi bậc.</p>
      <div className="mt-4 overflow-x-auto"><table aria-label="Kỹ năng Star" className="w-full min-w-[48rem] text-sm"><thead><tr><th className={cell}>Kỹ năng</th><th className={cell}>Nhóm</th><th className={cell}>Số bậc</th><th className={cell}>Tổng Star</th><th className={cell}>Khả dụng hiện tại</th></tr></thead><tbody>{data.perks.map((row) => <tr key={row.id}><th scope="row" className={cell}>{row.name}<code className="mt-1 block text-xs font-normal text-nova-muted">{row.id}</code></th><td className={cell}>{row.group}</td><td className={cell}>{row.levelPrices.length || 1}</td><td className={cell}>{row.maxCost}</td><td className={cell}><strong>{availabilityLabels[row.availability.status]}</strong><p className="mt-1 text-nova-muted">{row.availability.note}</p></td></tr>)}</tbody></table></div>
    </DstPanel>
    <DstPanel className="p-5 sm:p-6">
      ...
      <p aria-label="Nhận EXP nhiệm vụ mùa" className="mt-3 leading-7 text-nova-muted">Trong mùa, sau mỗi lần hoàn thành nhiệm vụ, kể cả mỗi lượt lặp lại, bạn bấm Claim EXP thủ công. Mỗi lần nhận thành công cấp EXP và ghi nhận 1 lượt nhận (claim). Khi chuyển mùa, hệ thống tự động nhận thưởng cho nhiệm vụ đã sẵn sàng và rương đủ điều kiện, lưu kết quả trước khi rút bộ nhiệm vụ mới. Nếu nhận thưởng thất bại, phần chưa nhận vẫn chờ xử lý, không bị mất khi đổi mùa. Lượng EXP theo cấu hình máy chủ; nếu máy chủ chưa cấu hình phần thưởng EXP thì chức năng nhận có thể chưa khả dụng.</p>
