...
riêng dưới `data/crawled/fandom-categories/<key>/`, nhưng dùng chung live
registry/cache dưới `<git-common-dir>/category-crawler/`. URL chưa có hoặc đang
`New` được claim; URL `Doing`/`Done` được skip network fetch. Crawler không mở
rộng đệ quy category con. Tracked registry JSON chỉ là review snapshot, không
phải live state.

Khởi tạo/audit live state trước batch đầu, rồi export snapshot sau batch:
...
# ... chạy một hoặc nhiều Category ...

python3 -m tools.crawl_wiki.state_cli snapshot \
  --snapshot data/crawled/fandom-url-registry.json
```

Nếu audit báo artifact `Done` bị mất/hỏng, chạy repair có chủ đích rồi crawl
lại row `New`; repair không thay đổi `Doing`:
...
Để test với storage riêng, truyền cả
`--url-registry <path> --shared-page-cache <path>` cho category crawler hoặc
state CLI. Thiếu một path hoặc dùng hai cờ này với profile khác sẽ hard-fail.
Raw shared payload trong Git common directory không được commit/deploy.
...
--page-budget <positive integer>
--url-registry <path>
--shared-page-cache <path>
--worker-id <id>
--skip-images
--fresh
--retry-errors
